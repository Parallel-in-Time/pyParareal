...
defect_inf = np.zeros((nentries,maxiter))
defect_l2  = np.zeros((nentries,maxiter))
slopes     = np.zeros(nentries)
psr        = np.zeros(nentries)
...
for nn in range(nentries):
  ...
  psr[nn], a, b, c = psr_pmat.get_psr(verbose=True)
...
plt.semilogy(range(1,5), [slopes[0]**(val-1)*1.1*defect_l2[0,0] for val in range(1,5)], 'b--', label=r'$|| E ||_2^k$')
plt.semilogy(range(1,5), [psr[0]**(val-1)*1.1*defect_l2[0,0] for val in range(1,5)], 'b-.', label=r'$\sigma_{\epsilon}(E)^k$')
